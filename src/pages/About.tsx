...
export const About = () => {
  ...
  const serviceCategories = categories.filter(c => c !== '전체');
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4">
          {serviceCategories.map((item) => (
            <button 
              ...
              className="w-[31%] md:w-auto md:flex-1 bg-white p-3 md:p-6 rounded-xl text-center font-medium shadow-sm text-xs md:text-base break-keep hover:bg-sage-800 hover:text-white transition-colors cursor-pointer"
            >
              {item}{(item !== '가구') ? '공간' : ''}
            </button>
          ))}
        </div>
        <p className="text-center text-sage-600 text-xs md:text-sm mt-6 leading-relaxed">
